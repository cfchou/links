...
fun manager(atoms, atomss) client {
  receive {
      ...
      switch (a) {
        ...
        case Just(a) ->
          switch ((atoms, connective)) {
            case ([], _)  -> replaceChildren(<#/>, getNodeById("filter"));
    ...
    case Reset ->
      replaceChildren(<#>true</#>, getNodeById("filter"));
      replaceChildren(<#/>, getNodeById("results"));
      replaceChildren(<#/>, getNodeById("price-error"));
      replaceChildren(<#/>, getNodeById("year-error"));
      manager([], [])
  }
}
...
fun lookupPrice() client {
  ...
  if (isPrice(s)) {
    ...
    replaceChildren(<#>Price must be a number greater than or equal to 0</#>, e);
    Nothing
  }
}
...
fun lookupYear() client {
  ...
  if (isYear(s)) {
    ...
    replaceChildren(<#>Year must be a year in the range 1970-2000</#>, e);
    Nothing
  }
}
