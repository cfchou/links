...
fun range(n) {
  if (n < 0) error("range: argument less than zero")
  else if (n==0) [0] else
  range(n-1) ++ [n]
}
...
# over queries.
sig firstField : (TableHandle (r, w, n), (r) {}-> Bool, (r) {}-> a) ~e~> (a::Base)
       ...
       { (formlet <#>
           <table>
            <tr><td class="label">Region:</td>
             <td>
              {choiceDefault(for (region <-- regionTable)
             ...
             <tr>
              ...
              <td>
               {choiceDefault(for (type <-- wineTypeTable)
